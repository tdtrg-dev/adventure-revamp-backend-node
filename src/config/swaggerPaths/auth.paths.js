...
module.exports = {
  ...
  '/forgot-password': {
    post: {
      tags: ['Auth'],
      summary: 'Forgot password, step 1 — email a 6-digit reset code',
      description:
  ...
  '/verify-reset-otp': {
    post: {
      tags: ['Auth'],
      summary: 'Forgot password, optional — check the code first and get a reset_token',
      description:
        'Only needed when the app checks the code on its own screen before asking for the new password; otherwise send the code straight to /reset-password. Codes allow 5 attempts. The returned reset_token is single-use and valid for 15 minutes.',
  ...
  '/reset-password': {
    post: {
      tags: ['Auth'],
      summary: 'Forgot password, step 2 — set a new password with the emailed code',
      description:
        'Send the 6-digit code from /forgot-password as otp (token is accepted as an alias) together with the new password. Alternatively send a reset_token from /verify-reset-otp instead of the code — exactly one of the two. Signs out every existing session on the account and marks the email verified if it was not. No accessToken is returned; the user logs in with the new password.',
      ...body('AuthResetPassword'),
      responses: {
        200: ok('Password updated.'),
        400: err(
          'Email not registered, invalid input, the code is incorrect, expired, or no longer active (data.otp), or the reset_token is invalid, expired, or already used (data.reset_token).'
        ),
        429: err('Too many incorrect code attempts — request a new code.'),
      },
    },
  },
  '/verify-email-otp': {
